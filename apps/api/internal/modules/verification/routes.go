...
func SetupAdminRoutes(router fiber.Router, db *sqlx.DB, log *zap.Logger, val *validator.Validator, notifSvc notification.Service) {
	repo := NewRepository(db)
	svc := NewService(repo, log, val, notifSvc)
	h := NewHandler(svc)
