...
func SetupAdminRoutes(router fiber.Router, db *sqlx.DB, log *zap.Logger, val *validator.Validator, notifSvc notification.Service, cfg *config.Config) {
	repo := NewRepository(db)
	svc := NewService(repo, log, val, notifSvc, cfg)
	h := NewHandler(svc)
