...
const rejectSchema = z.object({
  rejection_reason: z.string().min(5, 'Reason must be at least 5 characters'),
});
...
export function ParticipantActionMenu({ participantId, status, onViewDetail }: ParticipantActionMenuProps) {
  ...
  const mutation = useMutation({
    mutationFn: (variables: { status: 'APPROVED' | 'REJECTED', rejection_reason?: string }) => 
      participantService.verifyParticipant(participantId, variables),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['participants'] });
      queryClient.invalidateQueries({ queryKey: ['participant-summary'] });
      setModalType('NONE');
      setIsOpen(false);
      reset();
    }
  ...
  const handleReject = (data: RejectFormValues) => {
    mutation.mutate({ status: 'REJECTED', rejection_reason: data.rejection_reason });
  };
      ...
      {modalType !== 'NONE' && (
        <div className="fixed inset-0 bg-slate-900/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            {modalType === 'APPROVE' && (
              <>
                <h3 className="text-lg font-bold text-slate-900 mb-2">Approve Participant?</h3>
                <p className="text-slate-500 mb-6 text-sm">
                  Participant will become eligible for the next event phase. This action cannot be easily undone.
                </p>
            ...
            {modalType === 'REJECT' && (
              <form onSubmit={handleSubmit(handleReject)}>
                <h3 className="text-lg font-bold text-slate-900 mb-2">Reject Participant?</h3>
                <p className="text-slate-500 mb-4 text-sm">
                  Please provide a reason for rejecting this participant application.
                </p>
