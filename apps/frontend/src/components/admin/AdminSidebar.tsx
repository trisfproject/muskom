  ...
  return (
    <aside className="w-64 pg-bg border-r pg-border pg-muted flex flex-col h-screen sticky top-0 shrink-0 select-none">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        {navItems.map((group) => (
          <div key={group.title}>
            <p className="px-3 text-[11px] font-semibold uppercase tracking-wider pg-muted mb-2">
              {group.title}
            </p>
                  ...
                  <Link
                    ...
                    className={`flex items-center justify-between px-3 py-2 rounded-lg text-xs font-medium transition-all ${
                      isActive
                        ...
                        : "pg-muted hover:pg-text hover:pg-surface-elevated"
                    }`}
                  >
                    <div className="flex items-center gap-2.5">
                      <Icon className={`w-4 h-4 ${isActive ? "text-white" : "pg-muted group-hover:pg-text"}`} />
