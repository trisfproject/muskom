...
export default function AuditPage() {
  ...
  const columns: ColumnDef<AuditLog>[] = [
    ...
    },
    {
      ...
      header: "Modul",
      render: (row) => (
        <span className="inline-flex items-center px-2 py-1 rounded-md text-[10px] font-medium bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
          {row.module}
        </span>
      )
    },
    {
      ...
      render: (row) => (
        <div>
          <div className="font-medium pg-text">{row.action}</div>
