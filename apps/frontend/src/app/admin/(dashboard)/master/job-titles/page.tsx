...
import {
  Plus, Search, Pencil, Trash2, RotateCcw, Briefcase,
  ChevronLeft, ChevronRight, X, CheckCircle2, XCircle
} from "lucide-react";
import {
  adminMasterDataService, JobTitle, CreateJobTitleRequest,
...
function JobTitleForm({ initial, onSubmit, loading }: {
  initial?: Partial<CreateJobTitleRequest>;
  onSubmit: (data: CreateJobTitleRequest) => Promise<void>;
  loading: boolean;
}) {
  const [form, setForm] = useState<CreateJobTitleRequest>({
    name: initial?.name ?? "",
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="text-xs font-semibold pg-muted mb-1.5 block">Nama Jabatan *</label>
        <input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))}
          className="w-full px-3.5 py-2.5 rounded-xl border pg-border pg-bg pg-text text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" placeholder="Contoh: Direktur Utama" />
      </div>
      <div>
        <label className="text-xs font-semibold pg-muted mb-1.5 block">Urutan (Opsional)</label>
...
export default function JobTitlesPage() {
  ...
  const fetch = useCallback(async () => {
    setLoading(true);
    try {
      const res = await adminMasterDataService.listJobTitles({
        search, page, limit, ...(showDeleted ? {} : { is_active: undefined }),
      });
      setItems(res?.items ?? []);
  ...
  const handleCreate = async (data: CreateJobTitleRequest) => {
    setSaving(true);
    try {
      await adminMasterDataService.createJobTitle(data);
  ...
  const handleUpdate = async (data: CreateJobTitleRequest) => {
    if (!editing) return;
    setSaving(true);
    try {
      await adminMasterDataService.updateJobTitle(editing.id, data);
  ...
  const handleDelete = async (item: JobTitle) => {
    try {
      await adminMasterDataService.deleteJobTitle(item.id);
  ...
  const handleRestore = async (item: JobTitle) => {
    try {
      await adminMasterDataService.restoreJobTitle(item.id);
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-xl font-bold pg-text">Jabatan</h1>
          </div>
          <p className="text-sm pg-muted">Kelola data jabatan/posisi di perusahaan</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b pg-border pg-muted">
                <th className="text-left px-4 py-3 font-semibold">Nama Jabatan</th>
              ...
              {loading ? (
                [...Array(5)].map((_, i) => (
                  <tr key={i} className="border-b pg-border">
                    {[...Array(4)].map((_, j) => <td key={j} className="px-4 py-3"><div className="h-4 pg-border rounded animate-pulse" /></td>)}
                  </tr>
                ))
              ) : items.length === 0 ? (
                <tr><td colSpan={4} className="px-4 py-12 text-center pg-muted text-sm">
                ...
                <tr key={item.id} className={`border-b pg-border hover:pg-surface-elevated transition-colors`}>
                  ...
                  <td className="px-4 py-3 pg-muted">{item.sort_order}</td>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Tambah Jabatan">
        <JobTitleForm onSubmit={handleCreate} loading={saving} />
      </Modal>
      <Modal open={!!editing} onClose={() => setEditing(null)} title="Edit Jabatan">
        {editing && (
          <JobTitleForm
            initial={{ name: editing.name, sort_order: editing.sort_order, is_active: editing.is_active }}
            onSubmit={handleUpdate} loading={saving} />
        )}
      </Modal>
      <Modal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="Hapus Jabatan">
        {deleteTarget && (
