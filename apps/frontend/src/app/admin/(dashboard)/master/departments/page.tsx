...
import {
  Plus, Search, Pencil, Trash2, RotateCcw, Building2,
  ChevronLeft, ChevronRight, X, CheckCircle2, XCircle
} from "lucide-react";
import {
  adminMasterDataService, Department, CreateDepartmentRequest,
...
function DepartmentForm({ initial, onSubmit, loading }: {
  initial?: Partial<CreateDepartmentRequest>;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="text-xs font-semibold pg-muted mb-1.5 block">Nama Departemen *</label>
        <input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))}
          className="w-full px-3.5 py-2.5 rounded-xl border pg-border pg-bg pg-text text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" placeholder="Contoh: Human Resources" />
      </div>
...
export default function DepartmentsPage() {
  ...
  const fetch = useCallback(async () => {
    setLoading(true);
    try {
      const res = await adminMasterDataService.listDepartments({
  ...
  const handleCreate = async (data: CreateDepartmentRequest) => {
    setSaving(true);
    try {
      await adminMasterDataService.createDepartment(data);
  ...
  const handleUpdate = async (data: CreateDepartmentRequest) => {
    ...
    setSaving(true);
    try {
      await adminMasterDataService.updateDepartment(editing.id, data);
  ...
  const handleDelete = async (item: Department) => {
    try {
      await adminMasterDataService.deleteDepartment(item.id);
  ...
  const handleRestore = async (item: Department) => {
    try {
      await adminMasterDataService.restoreDepartment(item.id);
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-xl font-bold pg-text">Departemen</h1>
          </div>
          <p className="text-sm pg-muted">Kelola data departemen di perusahaan</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b pg-border pg-muted">
                <th className="text-left px-4 py-3 font-semibold">Nama Departemen</th>
              ...
              {loading ? (
                [...Array(5)].map((_, i) => (
                  <tr key={i} className="border-b pg-border">
                    {[...Array(3)].map((_, j) => <td key={j} className="px-4 py-3"><div className="h-4 pg-border rounded animate-pulse" /></td>)}
                  </tr>
                ))
              ) : items.length === 0 ? (
                <tr><td colSpan={3} className="px-4 py-12 text-center pg-muted text-sm">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Tambah Departemen">
        <DepartmentForm onSubmit={handleCreate} loading={saving} />
      </Modal>
      <Modal open={!!editing} onClose={() => setEditing(null)} title="Edit Departemen">
        {editing && (
          <DepartmentForm
            initial={{ name: editing.name, is_active: editing.is_active }}
            onSubmit={handleUpdate} loading={saving} />
        )}
      </Modal>
      <Modal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="Hapus Departemen">
        {deleteTarget && (
